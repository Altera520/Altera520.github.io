...
1. 다중 스레드 환경에서 Thread-safe를 보장해야하므로 stateless해야 한다. 즉, 싱글턴은 상태 정보를 클래스 내부에 가지고 있으면 안된다. 하지만, 클래스 내부에서 다른 싱글턴 인스턴스를 참조하는 경우는 가능하다.
    ```java
    public class Singleton {
    ...
    ```
1. 클래스 로더를 2개 이상 사용하면 싱글턴 인스턴스가 2개 이상 생성될수 있다.
