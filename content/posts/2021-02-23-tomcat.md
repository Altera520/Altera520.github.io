...
| **/webapps** | 웹 애플리케이션을 배포하는 기본 디렉토리이다. <br/> war파일을 webapps디텍토리 안에 위치시키면 하나의 Context로서 배포된다. <pre><code>    webapps<br/>        └── web application (web root)<br/>            ├── WEB-INF<br/>            │     ├── classes<br/>            │     ├── lib<br/>            │     └── web.xml<br/>            │<br/>            ├── META-INF<br/>            │     └── web.xml<br/>            │<br/>            └── *.html / *.jsp </code></pre><ul><li>**classes** : 서블릿 파일(.class)들이 위치한 서블릿 디렉터리다.</li><li>**lib** : jar 라이브러리들이 위치한다.</li></ul>|
| /work | 컴파일된 파일 |
{{% /table %}}
